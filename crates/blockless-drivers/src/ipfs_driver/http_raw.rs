...
impl HttpRaw {
    ...
    fn boundary_begin(boundary: &str) -> Vec<u8> {
        ...
        buf.write(EOL).unwrap();
        buf.write(b"Content-Type: application/octet-stream").unwrap();
        buf.write(EOL).unwrap();
        buf.write(EOL).unwrap();
        buf
    }
    ...
    pub async fn write_boundary(&mut self, val: &[u8]) -> Result<u64, IpfsErrorKind> {
        let tcp_stream = self.tcp_stream.as_mut().ok_or(IpfsErrorKind::RequestError)?;
        ...
        let mut buf = Vec::new();
        buf.write(format!("Content-Length: {}", body_buf.len()).as_bytes()).unwrap();
        buf.write(EOL).unwrap();
        buf.write(format!("Content-Type: multipart/form-data; boundary={}", boundary).as_bytes()).unwrap();
        buf.write(EOL).unwrap();
        buf.write(EOL).unwrap();
    ...
    pub async fn write_all(tcp_stream: &mut TcpStream, v: Vec<u8>) -> Result<(), IpfsErrorKind> {
        ...
        tcp_stream.write_all(&v).await.map_err(|_| IpfsErrorKind::RequestError)?;
        Ok(())
    }
    ...
    pub async fn read_response(&mut self)  -> Result<(u16, Vec<u8>), IpfsErrorKind>  {
        ...
        for i in 1..10 {
            ...
            let n = tcp_stream.read_buf(&mut buf).await.map_err(|_| IpfsErrorKind::RequestError)?;
            readn += n;
        ...
        let (pos, len) = loop {
            let parsed = httparse::parse_chunk_size(&bulk[parsed_pos..]).map_err(|_| IpfsErrorKind::RequestError)?;
            match parsed {
        ...
        Ok((status_code, bulk.split_off(parsed_pos+pos).split_to(len as _).to_vec()))
    }
    ...
    pub async fn connect(&mut self) -> Result<(), IpfsErrorKind> {
        ...
        stream.write_all(&headers).await.map_err(|_| IpfsErrorKind::RequestError)?;
