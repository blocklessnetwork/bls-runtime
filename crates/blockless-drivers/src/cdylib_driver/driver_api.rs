...
impl DriverApi {
    ...
    pub fn blockless_open(&self, uri: &str, opts: &str, fd: &mut u32) -> u32 {
        unsafe {
            let uri_len: u32 = uri.len() as _;
            let opts_len: u32 = opts.len() as _;
            (self.api_open)(
                uri.as_ptr(),
                uri_len,
                opts.as_ptr(),
    ...
    pub fn blockless_read(&self, fd: u32, buf: &mut [u8], rn: &mut u32) -> u32 {
        unsafe {
            let buf_len: u32 = buf.len() as _;
            (self.api_read)(fd, buf.as_mut_ptr(), buf_len, rn as *mut u32)
        }
    }

    pub fn blockless_write(&self, fd: u32, buf: &[u8], wn: &mut u32) -> u32 {
        unsafe {
            let buf_len: u32 = buf.len() as _;
            (self.api_write)(fd, buf.as_ptr(), buf_len, wn as *mut u32)
        }
    }
