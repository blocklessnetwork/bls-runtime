...
use wasmtime_cli_flags::{opt::WasmtimeOptionValue, CommonOptions as C};
...
#[derive(Parser, PartialEq, Clone, Default)]
pub struct WasmConfigure {
...
pub struct WasiConfigure {
    ...
    pub inherit_env: Option<bool>,
    pub listenfd: Option<bool>,
    pub tcplisten: Option<bool>,
}

#[derive(Parser, PartialEq, Clone)]
pub struct CommonOptions {
    #[command(flatten)]
    pub wasm: WasmConfigure,
    #[command(flatten)]
    pub wasi: WasiConfigure
}
...
impl RunCommon {
    ...
    pub fn load_module_contents(
        ...
        Ok(match engine.detect_precompiled(bytes) {
            ...
            None => {
                println!("111");
                RunTarget::Core(Module::from_file(engine, path)?)
            }
        })
    }
